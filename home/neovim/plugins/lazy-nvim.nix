...
in
{
  ...
  config =
    let
      ...
      debug = x: lib.debug.traceSeqN 2 x x;

      noop = x: x;
      ifNull = v: fn: if v == null then fn else noop;
      ...
      inferName =
        ...
        plugin:
        let
          plugin.name = if package != null then getName package else baseNameOf dir;
        in
        plugin;
      ...
      setPluginDir =
        package: plugin:
        let
          plugin.dir = toString package;
        in
        plugin;

      removePackageAttr = plugin: removeAttrs plugin [ "package" ];

      handleDependencies =
        { dependencies, ... }@plugin:
        let
          plugin.dependencies = finaliseSpec dependencies;
        in
        plugin;
