do
	...
	batch({
		-- How to exit vim
		{ "n", "<C-w>q", "<cmd>qa<cr>", { desc = "Exit only no buffer has unsaved changes" } },
		{ "n", "<C-w>x", "<cmd>xa!<cr>", { desc = "Save all changes and exit" } },
		{ "n", "<C-w>d", "<cmd>qa!<cr>", { desc = "Exit without saving changes (risky)" } },

		-- Fast saving
		{ "n", "<leader>w", "<cmd>up<cr>", { desc = "Save current modification (if any)" } },
		{ "n", "<leader>W", "<cmd>w<cr>", { desc = "Save current buffer (regardless of if changed or not)" } },

		-- Disable recent search highlights
		{ "n", "<leader><cr>", "<cmd>noh<cr>", { desc = "Disable recent search highlights" } },
		...
		{ "n", "<leader>tc", "<cmd>tabclose<cr>", { desc = "Close all but current tab" } },
		{ "n", "<leader>tm", "<cmd>tabmove", { desc = "Move a tab" } },
		{ "n", "<Leader>tl", '<cmd>exe "tabn ".g:lasttab<cr>', { desc = "Go to last accessed tab" } },
		{ "n", "<C-PageUp>", "<cmd>tabprevious<cr>", { desc = "Go to previous tab" } },
		...
		{ "v", "<M-j>", ":m'>+<cr>`<my`>mzgv`yo`z", { desc = "Move line(s) down" } },

		-- Sort selection
		{ "v", "<M-s>", ":sort<cr>", { desc = "Sort lines" } },

		-- Spell checking
		{ "n", "<leader>ss", "<cmd>setlocal spell!<cr>", { desc = "Toggle spellcheck on/off" } },
		{ "n", "<leader>sn", "]s", { desc = "Go to next spelling mistake" } },
		{ "n", "<leader>sp", "[s", { desc = "Go to previous spelling mistake" } },
		{ "n", "<leader>sa", "zg", { desc = "Add current locale's custom dictionary" } },
		{ "n", "<leader>s?", "z=", { desc = "Show a list of potential fixes for current spelling mistake" } },
	})
end
